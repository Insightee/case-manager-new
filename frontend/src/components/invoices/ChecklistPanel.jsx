...
export function ChecklistPanel({ items, onToggle }) {
  ...
  const feedback = useMemo(() => {
    if (remaining <= 0) return 'Payout workflow complete'
    if (remaining === 1) return '1 step left to complete payout'
    return `${remaining} steps left to complete payout`
  }, [remaining])
  ...
  return (
    <aside className="rounded-2xl border border-[#E2E8F0] bg-white p-5 shadow-[0_4px_24px_rgba(15,23,42,0.06)]">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h3 className="text-base font-semibold text-slate-900">Invoice progress checklist</h3>
          <p className="mt-1 text-sm text-slate-500">Finance readiness</p>
        </div>
