...
import { inviteRowResendHint } from '../../../lib/userProvision.js'
...
export function AdminInviteRowActions({
  ...
  const resendHint = inviteRowResendHint(invite)
  const forceResendNeeded = Boolean(invite?.expired_due_to_delivery_failure)
  ...
  async function resend(force = false) {
    ...
    setResending(true)
    try {
      const qs = force ? '?force_resend=true' : ''
      const res = await apiFetch(`/api/v1/admin/invites/${invite.id}/resend-email${qs}`, {
        method: 'POST',
      })
      if (res.skipped) {
        const hint = resendHint || 'Resend skipped — try again later or use force resend after correcting the email.'
        onError?.(hint)
        setActionMessage('Resend skipped')
        return
      }
      ...
      if (res.email_delivery === 'skipped_no_smtp') {
        ...
        onSuccess?.(msg)
        setActionMessage('Invite email queued')
      }
      onReload?.()
    } catch (err) {
